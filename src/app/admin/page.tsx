  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div>
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-emerald-900 mt-1">Dashboard</h1>
      </div>

      {/* Stats grid */}
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        {stats.map(({ label, value }) => (
          <Card key={label} className="bg-white rounded-2xl border border-gray-200 px-4 py-4 sm:px-6 sm:py-6 ring-0 gap-0">
            <CardContent className="p-0">
              <p className="text-xs uppercase tracking-widest text-gray-400 leading-tight">{label}</p>
        ...
        <CardContent className="p-0">
          ...
          <ul className="divide-y divide-gray-100">
            {recentActivity.map(({ id, text, meta }) => (
              <li key={id} className="flex items-start gap-3 px-4 sm:px-6 py-4">
                <div className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-emerald-900" />
                <div className="min-w-0">
                  <p className="text-sm font-medium text-emerald-900 wrap-break-word">{text}</p>
